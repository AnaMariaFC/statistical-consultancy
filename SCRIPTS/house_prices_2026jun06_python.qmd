...
```{r}
# Carregando os dados ...
dados_train<-read.csv("/home/delfos/Documentos/UEPB_20260326/PEDRO_MONTEIRO/CONSULTANCY/CONSULTANCY_PROJECTS/HOUSE_PRICES_PROJECT/DATA/house-prices-advanced-regression-techniques/train.csv")
...
Verificando dimensões da base

```{r}
dim(dados_train)  #Number line e col
str(dados_train)  #Tipo de variável
summary(dados_train)
```
...
```{r}
# Ordenando os dados faltantes
sort(colSums(is.na(dados_train)),
     decreasing=TRUE)

#is.na(): Cria uma matriz de TRUE (onde falta dados) e FALSE (onde o dado existe).
#colSums(): Soma os valores por coluna. Como o R entende TRUE como 1 e FALSE como 0,essa soma resulta no total de dados faltantes.
#sort(..., decreasing = TRUE): Pega a lista de contagem de NAs e a organiza em ordem decrescente (do maior para o menor).
...
```{python}
# Via Bash: pip install pandas numpy matplotlib seaborn scikit-learn
# Bem mais fácil!
...
#whitegrad: Fundo br c/line de grades
...
sns.histplot(dados_train['SalePrice'], kde=False, color='skyblue', ax=axes[0])
axes[0].set_title('Histograma de SalePrice')
axes[0].set_xlabel('Preço de Venda ($)')
axes[0].set_ylabel('Frequência')

# Densidade
sns.kdeplot(dados_train['SalePrice'], fill=True, color='purple', ax=axes[1])
axes[1].set_title('Gráfico de Densidade de SalePrice')
axes[1].set_xlabel('Preço de Venda ($)')
axes[1].set_ylabel('Densidade')

# Boxplot
sns.boxplot(y=dados_train['SalePrice'], color='lightgreen', ax=axes[2])
axes[2].set_title('Boxplot de SalePrice')
axes[2].set_ylabel('Preço de Venda ($)')
